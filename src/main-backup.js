...
class Portfolio3D {
  constructor() {
    ...
    this.isLowPowerMode = navigator.hardwareConcurrency < 4
    this.performanceMode = this.isReducedMotion || this.isLowPowerMode
    ...
    this.frameRate = this.performanceMode ? 30 : 60
  ...
  fallbackInit() {
    // Initialize without Three.js for better performance on low-end devices
  ...
  cacheDOM(selector, key = selector) {
  ...
  cacheDOMAll(selector, key = selector) {
    if (!this.domCache.has(key)) {
  ...
  setupThreeJS() {
    ...
    this.renderer = new THREE.WebGLRenderer({
      canvas: this.cacheDOM('#three-canvas'),
      antialias: !this.performanceMode, // Disable antialiasing for better performance
      alpha: true,
  ...
  createScene() {
    ...
    if (!this.performanceMode) {
      ...
      // Reduced point lights for performance
      const pointLight1 = new THREE.PointLight(0x00d4ff, 0.8, 10)
      pointLight1.position.set(-3, 2, 3)
      this.scene.add(pointLight1)
    }

    // Create floating geometric shapes (reduced count for performance)
    this.createFloatingGeometry()
    
    // Only create project previews if not in performance mode
    if (!this.performanceMode) {
  ...
  createFloatingGeometry() {
    ...
    // Reduce object count based on performance mode
    const objectCount = this.performanceMode ? 6 : 12
    ...
    for (let i = 0; i < objectCount; i++) {
      ...
      mesh.userData = {
        ...
        floatSpeed: Math.random() * 0.01 + 0.005, // Reduced speed
        floatRange: Math.random() * 1 + 0.5 // Reduced range
      }
  ...
  createProjectPreviews() {
    ...
    // Only create previews if not in performance mode
    if (this.performanceMode) return
    
    // Create 3D previews for each project
    for (let i = 1; i <= 6; i++) {
      const container = this.cacheDOM(`#project-preview-${i}`)
      ...
      switch(i) {
        case 1:
          geometry = new THREE.TorusKnotGeometry(0.8, 0.3, 50, 8) // Reduced segments
          ...
          break
        case 2:
          geometry = new THREE.SphereGeometry(1, 16, 16) // Reduced segments
          material = new THREE.MeshPhongMaterial({ 
            color: 0xff6b35,
            transparent: true,
            opacity: 0.8
          ...
          break
        case 3:
          geometry = new THREE.CylinderGeometry(0.5, 1, 1.5, 6) // Reduced segments
          material = new THREE.MeshPhongMaterial({ 
            color: 0x7c3aed,
            flatShading: true
          })
